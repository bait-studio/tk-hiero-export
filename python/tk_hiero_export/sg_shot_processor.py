...
from .sg_copy_exporter import ShotgunCopyExporter
from .sg_symlink_exporter import ShotgunSymLinkExporter
...
class ShotgunShotProcessorUI(
    ShotgunHieroObjectBase, ShotProcessorUI, CollatingExporterUI
):
    """
    ...
    def populateUI(self, *args, **kwargs):
        ...
        CollatingExporterUI.populateUI(
            self, collating_widget, properties, cut_support=self._cutsSupported()
        )
    ...
    def _build_tag_selector_widget(self, items, properties):
        ...
        templates = [
            t["code"]
            for t in self.app.shotgun.find("TaskTemplate", filter, fields=fields)
        ]
    ...
    def _get_all_tags_by_name(self):
        ...
        for project in projects:
            tagsByName.update(
                dict([(tag.name(), tag) for tag in hiero.core.findProjectTags(project)])
            )
        return tagsByName
...
class ShotgunShotProcessor(ShotgunHieroObjectBase, FnShotProcessor.ShotProcessor):
    ...
    def startProcessing(self, exportItems, preview=False):
        ...
        if self.app.get_nuke_version_tuple() >= (10, 5, 1) and preview:
            return FnShotProcessor.ShotProcessor.startProcessing(
        ...
        exportTemplate.insert(
            0, (".shotgun", ShotgunShotUpdaterPreset(".shotgun", properties))
        )
    ...
    def _getCutData(self, hiero_sequence):
        ...
        except TankHookMethodDoesNotExistError as e:
        ...
        prev_cut = sg.find_one(
            "Cut",
            [["code", "is", hiero_sequence.name()], ["entity", "is", parent_entity]],
            ["revision_number"],
    ...
    def _override_frame_server_check(self):
        ...
        try:
            ...
            self._real_frame_server_check = (
                hiero.ui.nuke_bridge.FnNsFrameServer.isServerRunning
            )
    ...
    def _processCut(self, cut_related_tasks):
        ...
        for (shot_updater_task, shot_process_task) in cut_related_tasks:
            ...
            tc_cut_item_in = self._timecode(
                cut_item_data["cut_item_in"], fps, drop_frame
            )
            tc_cut_item_out = self._timecode(
                cut_item_data["cut_item_out"], fps, drop_frame
            )
...
class ShotgunShotProcessorPreset(
    ...
    def __init__(self, name, properties):
        ...
        default_properties["sg_status_hiero_tags"] = [
            ("Ready To Start", "rdy"),
            ("In Progress", "ip"),
            ("On Hold", "hld"),
            ("Final", "fin"),
        ...
        default_properties["task_template_map"] = [
            ("Ready To Start", default_template),
        ...
        custom_properties = (
            self._get_custom_properties("get_shot_processor_ui_properties") or []
        )
        ...
        explicit_constructor_properties = properties.get(
            "shotgunShotCreateProperties", {}
        )
    ...
    def addUserResolveEntries(self, resolver):
        ...
        self.app.log_debug(
            "Adding custom resolvers %s"
            % [ctf["keyword"] for ctf in custom_template_fields]
        )
        for ctf in custom_template_fields:
            resolver.addResolver(
                "{%s}" % ctf["keyword"],
                ctf["description"],
                lambda keyword, task: self.app.execute_hook(
